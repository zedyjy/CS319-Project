...
apirouter.post(
  ...
  async (req, res) => {
    try {
      const student_id = req.body.student_id;
      const companyEvaluationFormAverage =
        req.body.companyEvaluationFormAverage;
      const relatedToDepartment = req.body.relatedToDepartment;
      const supervisorHasEngineeringBackground =
        req.body.supervisorHasEngineeringBackground;
      ...
      if (
        ...
        // No file was uploaded
        res.status(400).json({ message: "No file / Form data uploaded" });
        return;
      }
      // Access the uploaded file using req.file
      const uploadedFile = req.file;
      ...
      //Grades Logic
      const grade = await GradingForm.findOne({ studentID: student_id });
      grade.companyEvaluationFormAverage = companyEvaluationFormAverage;
      grade.relatedToDepartment = relatedToDepartment === "True" ? true : false;
      grade.supervisorHasEngineeringBackground =
        supervisorHasEngineeringBackground === "True" ? true : false;
